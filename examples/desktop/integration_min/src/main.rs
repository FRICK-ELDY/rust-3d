            ...
            let _rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    ops: wgpu::Operations {
                        ...
                        store: wgpu::StoreOp::Store, // ← bool ではなく StoreOp
                    },
                    depth_slice: None, // ← 0.26 で追加
                })],
...
struct App {
    window: Option<&'static Window>,      // 'static 参照（最小実装の割り切り）
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            let attrs = WindowAttributes::default()
                .with_title("integration_min (wgpu 26 / winit 0.30)")
                .with_inner_size(PhysicalSize::new(1280, 720));

            // Window を生成して 'static に固定
        ...
        match (self.window, self.gpu.as_mut()) {
            (Some(window), Some(gpu)) => match event {
                ...
                WindowEvent::ScaleFactorChanged { scale_factor: _, ref mut inner_size_writer } => {
                    // 要: 引数。現在サイズを再指定しておく（戻り値は ()）
                    let _ = inner_size_writer.request_inner_size(gpu.size);
                    // 必要であればここで gpu.resize(gpu.size) を呼ぶこともできる
                }
...
fn main() -> Result<()> {
    let event_loop = EventLoop::new()?;   // 0.30: Result を返す
    let mut app = App::new();
    event_loop.run_app(&mut app)?;        // 0.30: run_app
    Ok(())
}
